...
const countUnfundedGames = GAMES_JSON.reduce((count, game)=> {
    return game.pledged < game.goal ? count + 1 : count;
}, 0);

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `
A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games. Currently, ${countUnfundedGames} ${countUnfundedGames === 1 ? 'game remains' : 'games remain'} unfunded. We need your help to fund these amazing games!
`;

// create a new DOM element containing the template string and append it to the description container
const descriptionEl = document.createElement('p');

descriptionEl.innerText = displayStr;
...
topGameEl.innerHTML = `<strong>${topGame.name}</strong> - $${topGame.pledged.toLocaleString()}`;
firstGameContainer.appendChild(topGameEl);


// do the same for the runner up item
const runnerUpEl = document.createElement("p");
runnerUpEl.innerHTML = `<strong>${runnerUpGame.name}</strong> - $${runnerUpGame.pledged.toLocaleString()}`;
secondGameContainer.appendChild(runnerUpEl);


// 🔑 Secret Key components
const component1 = topGame.name.split(" ")[0];       // first word of most funded game
const component2 = runnerUpGame.name.split(" ")[0];  // first word of second most funded game
const component3 = rest.length;                      // number of remaining games

// combine all components without spaces
const secretKey = `${component1}${component2}${component3}`;
